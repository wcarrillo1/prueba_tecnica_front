...
const   [allData, setAllData] = useState([]),
        ...
        datos = async () =>  {
            const response = await GetRout(`tipopersona/all`)
            setAllData((response.length) ? response : [])
        },
        One = async (data) => {
            const response = await PostRoute(`tipopersona/one`, { id : data.id })
            setOneData((response[0]) ? response[0] : [])
            setModal(true)
        },
        StoreUpdate = async (data) => {
            let response = []
            response = await PostRoute(`tipopersona/${!data.id ? 'store' : 'update' }`, data)
        ...
        Actions = async (data) => { 
             const response = await PostRoute(`tipopersona/${ data.estado === 1 ? 'destroy' : 'active'}`, { id : data.id })
             datos()
...
return(
        ...
        <Container>
          ...
          <div style={{
           ...
           <h1 className="text-center">Tipo Persona</h1>
            <Row>
              <Nav>
