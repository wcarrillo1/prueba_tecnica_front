...
const   [allData, setAllData] = useState([]),
        ...
        datos = async () =>  {
            const response = await GetRout(`departamento/all`)
            setAllData((response.length) ? response : [])
        },
        One = async (data) => {
            const response = await PostRoute(`departamento/one`, { id : data.id })
            setOneData((response[0]) ? response[0] : [])
            setModal(true)
        },
        StoreUpdate = async (data) => {
            let response = []
            response = await PostRoute(`departamento/${!data.id ? 'store' : 'update' }`, data)
        ...
        Actions = async (data) => { 
             const response = await PostRoute(`departamento/${ data.estado === 1 ? 'destroy' : 'active'}`, { id : data.id })
             datos()
